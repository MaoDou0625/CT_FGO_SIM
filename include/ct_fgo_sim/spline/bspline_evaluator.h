...
public:
    ...
    static Result<T> Evaluate(
        ...
        const T eps = T(1e-4);
        const T u_plus = u + eps;
        const T u_minus = u - eps;
        if (u_plus <= T(1.0) && u_minus >= T(0.0)) {
            const Sophus::SE3<T> pose_plus = EvaluatePose(u_plus, t0, t1, t2, t3);
            const Sophus::SE3<T> pose_minus = EvaluatePose(u_minus, t0, t1, t2, t3);
            ...
            out.v_world = out.pose.so3() * out.v_body;
        }
